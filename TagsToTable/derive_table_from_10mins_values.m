function derive_table_from_10mins_values(source_folder, save_folder, ccname, dict_obj)
%% ------------------------------------- Description ----------------------------------------
% Usage:
...
dict = dict_obj.tenmins;
% find all the algorithms containing ccname (algnames and vernums)
algnames = string(dict.Result_Name(contains(dict.Result_Name,ccname))');
vernums = string(dict.Version(contains(dict.Result_Name,ccname))');
value_colnames = {'id','srcfile', 'start_date_time', 'stop_date_time', 'duration_avail_s', 'max_cross_corr', 'lag_max', 'min_cross_corr', 'lag_min'};
...
for i=1:length(allfileinfo)
    ...
    maxcc_col = strcmp(resultfile.result_tagcolumns(algrow).tagname,'Value'); 
    maxcc = resultfile.result_tags(algrow).tagtable(:,maxcc_col);
    
    mincc_col = strcmp(resultfile.result_tagcolumns(algrow).tagname,'MinCrossCorr'); 
    mincc = resultfile.result_tags(algrow).tagtable(:,mincc_col); 
    
    maxlag_col = strcmp(resultfile.result_tagcolumns(algrow).tagname,'LagMax'); 
    maxlag = resultfile.result_tags(algrow).tagtable(:,maxlag_col);
    
    minlag_col = strcmp(resultfile.result_tagcolumns(algrow).tagname,'LagMin'); 
    minlag = resultfile.result_tags(algrow).tagtable(:,minlag_col); 
    
    
    tbl_value_sbj = array2table([string(repmat(resultfile.id,size(start_datetime,1),1)), string(repmat(resultfile.srcfile,size(start_datetime,1),1)), string(start_datetime),string(stop_datetime), dur_avail, maxcc, maxlag, mincc, minlag]);
...
if ~isempty(tbl_value_cohort)
    ...
    writetable(tbl_value_cohort, strcat(save_folder, '\', ccname,'_10mins_values_bap_v',dict_obj.bap_v,'.csv'));
end
